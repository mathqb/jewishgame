...
namespace JewHunter
{
    public partial class frmJewHunter : Form
    {
        // start the variables
        ...
        private void gameEngine(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                MessageBox.Show("Vous vous êtes fait chasser comme un bon goy avec un total de " + score + " kills !" + "\r" + "\r" + "Appuyez sur OK pour recommencer une partie");
                //resetGame();
                Application.Restart();
                Environment.Exit(0);
            }
            ...
            foreach (Control x in this.Controls)
            {
                ...
                foreach (Control j in this.Controls)
                {
                    ...
                    if ((j is PictureBox && j.Tag == "bullet") && (x is PictureBox && x.Tag == "jew"))
                    {
        ...
        private void makeZombies()
        {
            ...
            zombie.Tag = "jew"; // add a tag to it called zombie
        ...
        private void resetGame()
        {
            ...
            foreach(Control x in this.Controls)
            {
                    if (x.Size == new Size(71, 71) || x.Tag == "jew")
                    {
                        x.Controls.Remove(x);
                    }
            }
            ...
            playerHealth = 100; // this double variable is called player health
            speed = 10; // this integer is for the speed of the player
            ammo = 10; // this integer will hold the number of ammo the player has start of the game
            zombieSpeed = 3; // this integer will hold the speed which the zombies move in the game
            score = 0; // this integer will hold the score the player achieved through the game
            gameOver = false; // this boolean is false in the beginning and it will be used when the game is finished
        ...
        {
            // when this function is called it will make zombies in the game
            player.Image = Properties.Resources.up; // the default picture for the zombie is zdown
            player.Left = rnd.Next(0, 900); // generate a number between 0 and 900 and assignment that to the new zombies left 
            player.Top = rnd.Next(0, 800); // generate a number between 0 and 800 and assignment that to the new zombies top
            player.SizeMode = PictureBoxSizeMode.AutoSize; // set auto size for the new picture box
            player.BringToFront();
        }
    }
}
